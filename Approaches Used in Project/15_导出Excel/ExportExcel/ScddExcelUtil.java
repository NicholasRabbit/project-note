...
import com.by4cloud.platform.scdd.entity.ReportDailyV;
...
public class ScddExcelUtil {


	public static void generateExcel(String title, List<ReportDailyV> list, HttpServletResponse response, Map<String,String> map) throws Exception {
		...
		XSSFCellStyle redStyle = ExcelUtil.genRedContextStyle(workbook);


		//创建Excel
		//生成基本数据
		genExcel(title, list, sheet, titleStyle, contextStyle, contextRightStyle, map, boldStyle, redStyle);
	...
	public static void genExcel(String title, List<ReportDailyV> list, XSSFSheet sheet, XSSFCellStyle titleStyle, XSSFCellStyle contextStyle,
								 XSSFCellStyle contextRightStyle, Map<String, String> map, XSSFCellStyle boldStyle, XSSFCellStyle redStyle) {
		//设置宽度
		for (int i = 0; i < 10; i++) {
			if(i == 2){
		...
		for (int i = 0; i < list.size(); i++) {
			...
			Double result = actual - plan;
			if(result == 0.0)
				result = null;
			if(result != null && result < 0) //小于0设置红色字体
				ExcelUtil.setCell(8, result, row, redStyle);
			else
				ExcelUtil.setCell(8, result, row, contextStyle);
			...
			Double result2 = actual2 - plan2;
			if(result2 == 0.0)
				result2 = null;
			if(result2 != null && result2 < 0) //小于0需设置红色字体
				ExcelUtil.setCell(12, result2, row, redStyle);
			else
				ExcelUtil.setCell(12, result2, row, contextStyle);
			...
			Double result3 = actual3 - plan3;
			if(result3 == 0.0)
				result3 = null;
			if(result3 != null && result3 < 0)  //小于0需设置红色字体
				ExcelUtil.setCell(15, result3, row, redStyle);
			else
				ExcelUtil.setCell(15, result3, row, contextStyle);
			...
			Double result4 = actual4 - plan4;
			if(result4 == 0.0)
				result4 = null;
			if(result4 != null && result4 < 0)  //小于0需设置红色字体
				ExcelUtil.setCell(19, result4, row, redStyle);
			else
				ExcelUtil.setCell(19, result4, row, contextStyle);
